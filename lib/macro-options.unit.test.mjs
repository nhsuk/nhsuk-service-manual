import {
  ...
  getMacroOptions,
  getMacroOptionsJson
} from './macro-options.js'

describe('Macro options', () => {
  describe('getFrontendVersion', () => {
    it('returns required NHS.UK frontend version', () => {
      const group = 'components'

      expect(getFrontendVersion({ group, item: 'inset-text' })).toBe('10.1.0')
      expect(getFrontendVersion({ group, item: 'checkboxes' })).toBe('10.6.1')
      expect(getFrontendVersion({ group, item: 'radios' })).toBe('10.6.1')
    })

    it('returns undefined for non-component groups', () => {
      const group = 'patterns'

      expect(getFrontendVersion({ group, item: 'mini-hub' })).toBeUndefined()
      expect(getFrontendVersion({ group, item: 'start-page' })).toBeUndefined()
    })
  })
  ...
  describe('getMacroOptions', () => {
    ...
    describe('markdown rendering', () => {
      ...
      it('renders descriptions as markdown', () => {
        ...
        expect(output[0].params?.[17].description).toBe(
      ...
      it('renders nested options descriptions as markdown', () => {
        expect(output[0].params?.[15].params?.[2].params?.[0].description).toBe(
