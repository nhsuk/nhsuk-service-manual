...
const componentsReverse = new Map(
  Array.from(components.entries()).map(([key, value]) => [value, key])
)

/**
 * @param {MacroOption} option
 * @param {MacroOptionWithSlug} [parent]
 * @returns {MacroOptionWithSlug}
 */
function addSlugs(option, parent) {
  const slug = slugify(kebabCase(option.name))

  const updated = /** @type {MacroOptionWithSlug} */ ({
    ...option,
    slug,
    id: slug
  })
  ...
  if ((!option.isComponent || option.params) && parent?.slug) {
    updated.slug = `${parent.slug}-${updated.slug}`
  }

  if (option.params) {
    updated.params = option.params.map((param) => addSlugs(param, updated))
  }
...
/**
 * @param {MacroOptionWithSlug} option
 * @param {MacroOptionWithSlug} [parent]
 * @returns {MacroOptionWithSlug}
 */
 ...
 *
 * @param {MacroOptionWithSlug[]} options
 * @param {MacroOptionWithSlug} [parent]
 */
function getNestedOptions(options, parent) {
  return options
    ...
    .flatMap((option) => {
      /** @type {[MacroOptionWithSlug, MacroOptionWithSlug | undefined][]} */
...
/**
 * Get NHS.UK frontend version required for a component
 *
 * @param {Pick<FileOptions, 'group' | 'item'>} options
 */
function getFrontendVersion({ group, item }) {
  if (group !== 'components') {
    return
  }

  let /** @type {string | undefined} */ version

  for (const { params = [] } of getMacroOptions(getComponentName(item))) {
    for (const { released, deprecated } of params) {
      if (
        !version ||
        (!deprecated && !!released && semver.gt(released, version))
      ) {
        version = released
      }
    }
  ...
  return version ?? nhsukFrontendPkg.version
}
 ...
 * @returns {MacroOptionWithSlug[]} Macro options with slug
 */
function getMacroOptions(componentName) {
  ...
  const optionGroups = /** @type {MacroOptionWithSlug[]} */ ([
    {
      name: 'Primary options',
      slug: 'primary',
      id: 'primary',
...
/**
 * @typedef {Omit<MacroOption, 'type' | 'required' | 'description' | 'params'> & {
 *   slug: string,
 *   id: string,
 *   type?: MacroOption['type'],
 *   required?: MacroOption['required'],
 *   description?: MacroOption['description'],
 *   params?: MacroOptionWithSlug[]
 * }} MacroOptionWithSlug
 */

/**
 * @import { MacroOption } from 'nhsuk-frontend/lib'
 * @import { FileOptions } from './file-helper.js'
 */
